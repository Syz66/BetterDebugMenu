...
public class DebugUtils {
    ...
    public static List<String> getDebugInfoLeft() {
        ...
        String verticalFacing = getVerticalFacing(entity.rotationPitch);
        Chunk chunk = mc.theWorld.getChunkFromBlockCoords(blockpos);

        List<String> list;

        if (BetterDebugMenu.config.pitch) {
            list = Lists.newArrayList(
                    ...
                    "fv: " + verticalFacing + " / " + MathHelper.wrapAngleTo180_float(mc.thePlayer.rotationPitch),
    ...
    public static String getVerticalFacing(float pitch) {
        EnumFacing facing;
        int number;
        if (pitch > 0.0f) {
        ...
        return number + " (" + facing.toString().toUpperCase() + ")";
    }
}
